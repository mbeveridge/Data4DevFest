...
accidents %>% count(codigo_accidente)    # 9982 rows (unique). Seem to be 5e+05 or 4e+06
...
accidents %>%
  ...
  summarise(count = n()) %>%
  ggplot(aes(yday, count)) +                  # per 'day of the year' (0 is 1Jan, 365 is 31Dec)
  geom_point() +
  geom_smooth()
...
ggplot(accidents, aes(x, y, colour = localidad)) +        # a rough map of the 'localidads'
  geom_point()
```
